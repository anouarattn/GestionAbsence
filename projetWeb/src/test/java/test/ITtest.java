...
public class ITtest {
	...
	public static EnterpriseArchive createDeployment() {
		...
		JavaArchive jar = ShrinkWrap
				.create(ZipImporter.class, "projetEjb.jar")
				.importFrom(new File("./../projetEjb/target/projetEjb.jar"))
				.as(JavaArchive.class)
				.addAsResource("META-INF/test-persistence.xml",
						"META-INF/persistence.xml");

		EnterpriseArchive ear = ShrinkWrap
				.create(EnterpriseArchive.class, "projetEar.ear")
	...
	public void departement_add_modify_show() {
		...
		//
		// WebElement tableDataElement =
		// driver.findElementById(DATATABLE_TBODY_ID);
		//
		// // test that only one element exist in the datatable. the empty
		// message row.
		// Assert.assertEquals(
		// 1,tableDataElement.findElements(By.xpath("./tr")).size());
		// // assure only empty message exist
		// Assert.assertEquals(tableDataElement.findElements(By.xpath("./tr")).get(0).getAttribute("class"),
